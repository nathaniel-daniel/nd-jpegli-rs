...
pub fn exec(metadata: cargo_metadata::Metadata, _options: Options) -> anyhow::Result<()> {
    ...
    custom_jpegli_file_str.push_str("pub const JPEGLI_INCLUDE_DIR: &str = \"custom-jpegli/include/jpegli\";\n");
    ...
    custom_jpegli_file_str.push_str("pub const HIGHWAY_SRCS: &[&str] = &[\n");
    for file in highway_src_files {
        let file_components: Vec<_> = file
            .components()
            .map(|component| component.as_str())
            .collect();
